...
type APIKey struct {
	...
	Name    string
	GroupID *int64
	// OpenCodePoolID is populated when this key is bound to the system-managed
	// OpenCode worker pool. It is deliberately independent from GroupID: the
	// original group remains the billing/routing context for non-free models.
	OpenCodePoolID *int64
	Status         string
	IPWhitelist    []string
	IPBlacklist    []string
