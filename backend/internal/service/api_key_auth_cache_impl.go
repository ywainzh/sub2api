...
const apiKeyAuthSnapshotVersion = 20 // v20: merge OpenCode pool binding (release) with group search/audio/video_model_prices billing fields (upstream)
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		OpenCodePoolID: apiKey.OpenCodePoolID,
		Name:           apiKey.Name,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		OpenCodePoolID: snapshot.OpenCodePoolID,
		Key:            key,
		Name:           snapshot.Name,
